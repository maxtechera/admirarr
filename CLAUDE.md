...
.opencode/                     # OpenCode agent + command config
```
...
- **Claude Code plugin** (`.claude-plugin/`): marketplace for `/plugin install admirarr@admirarr`
- **OpenCode config** (`.opencode/opencode.jsonc`): agents + commands
- **Root `SKILL.md`**: single source of truth for all agent platforms
