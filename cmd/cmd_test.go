...
func TestSetupCommand_DryRunFlag(t *testing.T) {
	flag := setupCmd.Flags().Lookup("dry-run")
	if flag == nil {
		t.Fatal("setup command should have --dry-run flag")
	}
	if flag.DefValue != "false" {
		t.Errorf("expected --dry-run default=false, got %s", flag.DefValue)
	}
	if flag.Usage == "" {
		t.Error("--dry-run flag should have usage text")
	}
}

func TestSetupCommand_AutoFlag(t *testing.T) {
	flag := setupCmd.Flags().Lookup("auto")
	if flag == nil {
		t.Fatal("setup command should have --auto flag")
	}
	if flag.DefValue != "false" {
		t.Errorf("expected --auto default=false, got %s", flag.DefValue)
	}
}
