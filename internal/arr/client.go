...
// Artists fetches all artists from Lidarr.
func (c *Client) Artists() ([]Artist, error) {
	var data []Artist
	err := c.api.GetJSON(c.endpoint("artist"), nil, &data)
	return data, err
}

// --- Readarr ---

// Authors fetches all authors from Readarr.
func (c *Client) Authors() ([]Author, error) {
	var data []Author
	err := c.api.GetJSON(c.endpoint("author"), nil, &data)
	return data, err
}
