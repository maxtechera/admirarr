...
func tryBuiltinFix(issue Issue) bool {
	...
	// Gluetun unhealthy / VPN disconnected → check if it's a credentials issue, auto-fix with Mullvad
	...
	// Host mismatch warnings → update config automatically
	if strings.HasPrefix(desc, "WARNING") && strings.Contains(desc, "Found at") && strings.Contains(desc, "but configured as") {
		return fixHostMismatch(desc)
	}
...
// fixHostMismatch updates the config when a service is found at a different host
// than configured. Pattern: "WARNING [prowlarr]: Found at 192.168.50.42 but configured as localhost"
func fixHostMismatch(desc string) bool {
	// Extract service name: WARNING [<name>]:
	start := strings.Index(desc, "[")
	end := strings.Index(desc, "]")
	if start < 0 || end < 0 || end <= start {
		return false
	}
	service := desc[start+1 : end]

	// Extract actual host: "Found at <host>"
	foundIdx := strings.Index(desc, "Found at ")
	if foundIdx < 0 {
		return false
	}
	rest := desc[foundIdx+len("Found at "):]
	spaceIdx := strings.Index(rest, " ")
	if spaceIdx < 0 {
		return false
	}
	actualHost := rest[:spaceIdx]

	fmt.Printf("  %s Updating %s host: %s → %s\n", ui.GoldText("⟳"), service, "localhost", actualHost)
	config.SetServiceHost(service, actualHost)

	// Also update the config file on disk
	configPath := filepath.Join(os.Getenv("HOME"), ".config", "admirarr", "config.yaml")
	data, err := os.ReadFile(configPath)
	if err != nil {
		fmt.Printf("  %s Cannot read config: %v\n\n", ui.Err("✗"), err)
		return false
	}

	// Find and update the host line for this service
	...
	if err := os.WriteFile(configPath, []byte(strings.Join(lines, "\n")), 0644); err != nil {
		fmt.Printf("  %s Cannot write config: %v\n\n", ui.Err("✗"), err)
		return false
	}
	fmt.Printf("  %s Config updated for %s\n\n", ui.Ok("✓"), service)
	return true
}

// fixVPNCredentials auto-creates a Mullvad account, generates WireGuard keys,
// registers a device, writes credentials to .env, updates docker-compose.yml
// if needed, and restarts Gluetun.
func fixVPNCredentials() bool {
	fmt.Printf("  %s Gluetun needs WireGuard credentials — auto-provisioning via Mullvad API\n", ui.GoldText("⟳"))

	composeDir := filepath.Dir(config.Get().ComposePath)
	envPath := filepath.Join(composeDir, ".env")

	// Check if credentials already exist in .env but compose is stale
	if data, err := os.ReadFile(envPath); err == nil {
		if strings.Contains(string(data), "WIREGUARD_PRIVATE_KEY=") {
			for _, line := range strings.Split(string(data), "\n") {
				if strings.HasPrefix(line, "WIREGUARD_PRIVATE_KEY=") && strings.TrimPrefix(line, "WIREGUARD_PRIVATE_KEY=") != "" {
					// Credentials exist — compose might be stale
					fmt.Printf("  %s VPN credentials exist in .env — checking compose template\n", ui.Dim("→"))
	...
	// No credentials — create Mullvad account and register device
	client := vpn.NewClient()

	fmt.Printf("  %s Creating Mullvad account…\n", ui.GoldText("→"))
	acct, err := client.CreateAccount()
	if err != nil {
		fmt.Printf("  %s Cannot create Mullvad account: %v\n\n", ui.Err("✗"), err)
		return false
	}
	fmt.Printf("  %s Account: %s\n", ui.Ok("✓"), vpn.FormatAccountNumber(acct.Number))
	...
	// Ensure compose has VPN vars
	fixComposeVPNVars(composeDir)
...
// fixComposeVPNVars checks if docker-compose.yml has WIREGUARD_PRIVATE_KEY
// in the gluetun environment section. If not, regenerates the compose file.
func fixComposeVPNVars(composeDir string) bool {
	...
	// Compose is stale — extract services and regenerate
	fmt.Printf("  %s Compose missing VPN vars — regenerating\n", ui.GoldText("⟳"))

	// Parse existing services from compose
	var services []string
	for _, line := range strings.Split(string(data), "\n") {
		if strings.HasPrefix(line, "    container_name: ") {
			name := strings.TrimPrefix(line, "    container_name: ")
			// Map container name back to service name
			for _, svcName := range config.AllRegisteredNames() {
				if config.ContainerName(svcName) == name {
	...
	opts := migrate.ComposeOpts{
		...
		ConfigDir:   envVars["CONFIG_DIR"],
		TZ:          envVars["TZ"],
		PUID:        envVars["PUID"],
		PGID:        envVars["PGID"],
		VPNProvider: envVars["VPN_PROVIDER"],
		VPNType:     envVars["VPN_TYPE"],
	}
