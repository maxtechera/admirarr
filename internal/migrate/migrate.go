...
type Options struct {
	...
	DryRun         bool
	Services       []string
	RemoteServices []string // populated by Run() — services excluded from compose
	TZ             string
	PUID           string
	PGID           string
}
...
func Run(opts Options) *Result {
	...
	// Filter out remote/native services — only Docker containers belong in compose.
	// Remote services (e.g. Radarr/Sonarr on Windows) are managed outside Docker.
	opts.RemoteServices = config.RemoteServices(opts.Services)
	opts.Services = config.DockerOnlyServices(opts.Services)

	if len(opts.RemoteServices) > 0 {
		fmt.Printf("  %s Excluding remote services from compose: %s\n",
			ui.Dim("→"), strings.Join(opts.RemoteServices, ", "))
	}
...
func generateCompose(r *Result, opts Options) {
	...
	content := GenerateCompose(opts.Services, ComposeOpts{
		...
		PUID:           opts.PUID,
		PGID:           opts.PGID,
		RemoteServices: opts.RemoteServices,
	})
