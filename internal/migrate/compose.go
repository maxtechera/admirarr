...
type ComposeOpts struct {
	...
	RemoteServices []string // services running remotely, documented in compose header
}

// GenerateCompose builds a docker-compose.yml from selected services.
// Only Docker-local services should be passed — remote services are excluded.
func GenerateCompose(services []string, opts ComposeOpts) string {
	var b strings.Builder

	b.WriteString(composeHeader)

	// Note remote services in header for visibility
	if len(opts.RemoteServices) > 0 {
		b.WriteString(fmt.Sprintf("# Note: %s run remotely, not in Docker.\n\n",
			strings.Join(opts.RemoteServices, ", ")))
	}
