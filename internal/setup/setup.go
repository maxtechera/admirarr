...
// wouldFix logs what would be fixed in dry-run mode, or returns false to proceed with the fix.
func (s *SetupState) wouldFix(r *StepResult, format string, args ...interface{}) bool {
	if !s.DryRun {
		return false
	}
	msg := fmt.Sprintf(format, args...)
	fmt.Printf("  %s %s %s\n", ui.GoldText("~"), msg, ui.Dim("(dry-run)"))
	r.fix()
	return true
}
...
func runSetup(state *SetupState) {
	if state.DryRun {
		fmt.Printf("  %s %s\n", ui.GoldText("~"), ui.Bold("DRY RUN — no changes will be applied"))
		fmt.Println()
	}
...
func printFinalSummary(state *SetupState, results []StepResult) {
	fmt.Println()
	if state.DryRun {
		fmt.Printf("  %s %s\n", ui.GoldText("~"), ui.Bold("DRY RUN COMPLETE — run without --dry-run to apply"))
	} else {
