...
var keyedServices = []string{"jellyfin", "sonarr", "radarr", "prowlarr", "lidarr", "readarr", "whisparr", "seerr", "bazarr", "tautulli", "plex", "sabnzbd"}
...
func initJellyfin(state *SetupState, svc *ServiceState, r *StepResult) bool {
	...
	if state.wouldFix(r, "Jellyfin → Run startup wizard (create user + API key)") {
		return true
	}
