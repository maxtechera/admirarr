...
func checkArrDownloadClient(state *SetupState, r *StepResult, service, expectedCategory string) {
	...
	if qbitClient == nil {
		...
		if state.wouldFix(r, "%s → Create qBittorrent download client (category=%s)", titleCase(service), expectedCategory) {
			return
		}
	...
	var category string
	dc, _ := config.DownloadClientFor(service)
	...
	if state.wouldFix(r, "%s → Fix download client (%s)", titleCase(service), strings.Join(issues, ", ")) {
		return
	}
...
func createQbitClient(state *SetupState, r *StepResult, service, category string) bool {
	...
	dcDef, _ := config.DownloadClientFor(service)
	catField := dcDef.CategoryField
	if catField == "" {
		catField = "movieCategory"
	}
	schema.SetField(catField, category)
...
func checkQbitCategories(state *SetupState, r *StepResult) {
	...
	for catName, expectedPath := range expectedCategories {
		...
		if state.wouldFix(r, "qBittorrent → Fix category %s (path=%s)", catName, expectedPath) {
			continue
		}
...
func checkQbitDefaultPath(state *SetupState, r *StepResult) {
	...
	if state.wouldFix(r, "qBittorrent → Fix default save path (%s → %s)", prefs.SavePath, expectedPath) {
		return
	}
...
func checkDownloadDirs(state *SetupState, r *StepResult) {
	...
	for _, dir := range dirs {
		...
		if state.wouldFix(r, "Create directory %s", dir) {
			continue
		}
