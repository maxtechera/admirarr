...
func SyncQualityProfiles(state *SetupState) StepResult {
	...
	if rt.Method != "none" {
		...
		if cfgContent != "" {
			if state.wouldFix(&r, "Write recyclarr.yml config") {
				// skip
			} else if err := writeRecyclarrConfig(state, cfgContent); err != nil {
	...
	if rt.Method != "none" {
		if state.wouldFix(&r, "Run Recyclarr sync (TRaSH Guides profiles)") {
			// skip
		} else {
			syncRecyclarr(rt, &r)
		}
	...
	for _, svcName := range []string{"radarr", "sonarr", "lidarr", "readarr", "whisparr"} {
		...
		switch svcName {
		case "radarr":
			syncRadarrQuality(state, profile, &r)
		case "sonarr":
			syncSonarrQuality(state, profile, &r)
...
func syncRadarrQuality(state *SetupState, profile string, r *StepResult) {
	...
	if state.wouldFix(r, "Radarr → Update %d movies to profile %s", needsUpdate, profile) {
		return
	}
...
func syncSonarrQuality(state *SetupState, profile string, r *StepResult) {
	...
	if state.wouldFix(r, "Sonarr → Update %d series to profile %s", needsUpdate, profile) {
		return
	}
